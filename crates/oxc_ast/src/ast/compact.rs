...
/// # How it works
///
/// The macro has three phases:
///
/// ### 1. Entry
///
/// If all arms are variant arms, a simple repetition expands them into an exhaustive discriminant match.
/// Otherwise, a TT muncher is used.
/// The entry point also handles different target forms (`ident`, `&ident`, `&mut ident`,`expr`).
/// * `ident` forms: target is used directly (no `let` binding) so the original variable remains accessible
///   in catch-all guards.
/// * Complex expression (e.g. `x.foo()`): A `let target` binding avoids evaluating the expression more than once.
///
/// ### 2. Munch
///
/// Arms are processed one at a time and transformed into a single accumulator.
/// * Variant arms (`Type::Variant(binding) => body`) become discriminant pattern arms.
/// * `_` arms are passed through as-is.
/// * Binding arms (`name if guard => body`) use `matches!` with `ref` in the guard (to borrow rather than move
///   the target), and `let name = target` in the body.
///
/// Rules are tried in order: variant (`ident ::`) before `_` before binding (`ident`).
/// Each rule has 2 forms:
/// 1. Non-terminal form (expects `,` + more tokens).
/// 2. Terminal form (last arm, optional trailing comma).
///
/// ### 3. Emit
///
/// Outputs `match target.ty() { accumulated_arms }`.
#[macro_export]
macro_rules! match_compact {
    // ==========
    // Entry: Exhaustive case - all arms are variant arms.
    // Uses a simple repetition (no TT muncher needed).
    // ==========
    ($target:expr, {
        $($ty:ident :: $variant:ident ($binding:ident) => $body:expr),+
        $(,)?
    }) => {{
        use $crate::ast::compact::CompactEnum;
        let target = $target;
        match target.ty() {
    ...
    // ==========
    // Entry: Non-exhaustive case - has catch-all arms.
    // Dispatches to the TT muncher.
    // Different target forms are handled so that for identifiers, the original variable is used directly
    // (not moved into a `let` binding), keeping it accessible in catch-all guards.
    // ==========
    ...
    // `match_compact!(&mut foo, { ... })`
    (&mut $target:ident, { $($arms:tt)+ }) => {
        match_compact!(@munch [&mut $target] [] $($arms)+)
    };

    // Expression target e.g. `match_compact!(foo.bar(), { ... })`.
    ...
    // Last arm without trailing comma: Append a comma and re-invoke, so the normal rules match
    (@munch [$($target:tt)+] [$($acc:tt)*] $($last:tt)+) => {
        match_compact!(@munch [$($target)+] [$($acc)*] $($last)+,)
    };

    // Terminal: All arms consumed, emit the match
    (@munch [$($target:tt)+] [$($acc:tt)+]) => {{
        use $crate::ast::compact::CompactEnum;
        match ($($target)+).ty() {
            $($acc)+

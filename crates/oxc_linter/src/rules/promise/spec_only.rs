...
fn spec_only(prop_name: &str, member_span: Span) -> OxcDiagnostic {
    OxcDiagnostic::warn(format!("Avoid using non-standard `Promise.{prop_name}` method."))
        .with_help("Use only standard `Promise` methods (`all`, `allSettled`, `any`, `race`, `reject`, `resolve`, `withResolvers`). Non-standard methods may not be available in all environments.")

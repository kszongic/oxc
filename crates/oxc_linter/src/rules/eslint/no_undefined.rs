...
fn no_undefined_diagnostic(span: Span) -> OxcDiagnostic {
    OxcDiagnostic::warn("Unexpected use of `undefined`").with_help("Use `void 0` if you need to check for `undefined`, or restructure the code to avoid referencing `undefined` directly.").with_label(span)
}
